...
const orderProductSeedData = async (orders, products) => [
    {
        orderId: orders[random(1, orders.length - 1)].id,
        productId: products[random(1, products.length - 1)].id,
...
async function seedDatabase(tables) {
    ...
    if (tables.products.length == 0) {
        const userRaw = async () =>
            await models.User.findAll({ where: { role: 'vendor' } });
        // const users = await userRaw().then((res) =>
        //     res.map((item) => item.toJSON())
        // );
        const users = await getVendors();
        if (!users) return;
        await models.Product.bulkCreate(await productsSeedData(users));
    } else console.log(`Table Products has data. Skipping it..`);

    if (tables.orders.length == 0) {
        const usersRaw = async () =>
            await models.User.findAll({ where: { role: 'normal' } });
        // const users = await usersRaw().then((res) =>
        //     res.map((item) => item.toJSON())
        // );
    ...
    if (tables.orderProducts.length == 0) {
        ...
        const orders = await ordersRaw().then((res) =>
            res.map((item) => item.toJSON())
        );

        const productsRaw = async () => await models.Product.findAll({});
        const products = await productsRaw().then((res) =>
            res.map((item) => item.toJSON())
        );

        await models.OrderProduct.bulkCreate(
            await orderProductSeedData(orders, products)
        );
...
const getVendors = async () => {
    const usersRaw = await models.User.findAll({
        attributes: {
    ...
    const users = usersRaw.map((item) => item.toJSON());
    const users2 = users.map((user) => console.log(user.UserRoles));
    console.log(
        'User Roles: ',
        users.filter((user) => user.UserRoles[0]['Role']['name'] == 'vendor')
    );

    // return users.filter(
    //     (user) => user.UserRoles[0]['Role']['name'] == 'vendor'
    // );
};
